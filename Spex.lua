...
local function CreateSpecTable()
	--[[ Creates the tables for each specX where X is the spec number.
		 Tables contain a spec ID number and a text line to be used in the DropDownMenu. ]]--
	...
	--[[ Set the number of specs the current player class has.
		 Used as an iteration limit for creating the spec tables and drop down items. ]]--
	...
	for i = 1, Spex.numSpecs do
		...
		--[[ Retrieving name, icon, and role information from blizz api for current spec number ]]--
		local _, name, _, icon, role, _ = GetSpecializationInfo(i)
		
		--pline(name,icon,role)
		
		--[[ Sets ["text"] value to a string containing the role symbol, spec name, and spec icon ]]--
...
local function CreateDropDown(self, level)
	--[[ Creates the DropDownMenuItems and enables spec switching functionality ]]--
	...
	for i = 1, Spex.numSpecs do
		...
		dropDownMenuItem.func = function(self)
			--[[ Sets the DropDownMenuItem function to call SetSpecialization on the
				 selected spec if it is not the currently selected spec when clicked ]]--
...
local function CreateSpexFrame()
	...
	--[[ Checks for savedvariables in SpexDB and loads them into Spex.settings if they exist ]]--
	SpexDB = SpexDB or {}
	...
	if Spex.settings.XPos then
		...
		--[[ defaults location to center of screen if no existing savedvariables ]]--
	...
	--[[ when frame is moved, upon stop the position is saved to savedvariables in SpexDB ]]--
	Spex.SpexFrame:SetScript("OnDragStop", function(self)
												Spex.SpexFrame:StopMovingOrSizing()
												Spex.settings.XPos = Spex.SpexFrame:GetLeft()
												Spex.settings.YPos = Spex.SpexFrame:GetBottom()
											end); -- end function(self)/SetScript
	
	--[[ creates the DropDownMenuItems then assigns basic DropDownMenu configurations ]]--
...
local function HandleEvent(self, event, ...)
	--[[ triggered by registered events and sets off the creation of spex table & frame
		 and triggers DropDownMenu updates ]]--
	
	--pline("Entering HandleEvent() on "..event)
	
	
	if not Spex["spec1"] then
		--[[ checks for existing spec table, if none found creates one.  prevents duplicates ]]--
		CreateSpecTable()
	end -- end if not Spex["spec1"]
	
	if not Spex.SpexFrame then
		--[[ checks for existing spexframe, if none found creates one.  prevents duplicates ]]--
		CreateSpexFrame()
